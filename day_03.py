...
xforms = {
    'R': lambda pos, val: (pos[0] + val, pos[1]),
    'L': lambda pos, val: (pos[0] - val, pos[1]),
    'U': lambda pos, val: (pos[0], pos[1] + val),
    'D': lambda pos, val: (pos[0], pos[1] - val)
}

Point = collections.namedtuple('Point', ['x', 'y'])
RawSegment = collections.namedtuple('RawSegment', ['a', 'b'])

_Segment = collections.namedtuple('Segment', ['segment', 'steps'])


class Segment(_Segment):
    @classmethod
    def __new__(cls, a, b, steps=None):

        segment = RawSegment(a, b)

        return super(Segment, cls).__init__(segment, steps)

    def __init__(self, segment, steps=None):
        import pdb
        pdb.set_trace()
        self.segment = segment
        self.a = segment.a
        self.b = segment.b
        super(Segment, self).__init__(segment, steps)

    def __iter__(self):
        return self.segment


def _generate_segments(path):

    # steps at beginning of segment
    steps = 0
    segments = set()
    ...
    for instruction in path.split(','):
        ...
        position = xforms[direction](last_position, val)
        segments.add(Segment(last_position, position, steps))
...
def _get_intersection(seg_a, seg_b):

    slope_a, slope_b = (int(seg_a.a.x == seg_a.b.x), int(seg_b.a.x == seg_b.b.x))

    if slope_a == slope_b:
        if slope_a == 0:
            a_x = sorted([p[0] for p in seg_a])
            b_x = sorted([p[0] for p in seg_b])

            if (a_x[1] > b_x[0] or b_x[1] > a_x[0]):
                intersection = (min([a_x[1], b_x[1]]), max(a_x[0], b_x[0]))
                import pbb
                pdb.set_trace()
    ...
    # I coulda done this a lot cleaner if I remembered my linear alg
    elif slope_a:
        # a is vertical, b is horizontal
        a_x = seg_a[0][0]
        a_y = sorted(p[1] for p in seg_a)
        b_x = sorted(p[0] for p in seg_b)
        b_y = seg_b[0][1]

        if  b_x[0] <= a_x <= b_x[1] and a_y[0] <= b_y <= a_y[1]:
            return (a_x, b_y)
        ...
        a_y = seg_a[0][1]
        b_x = seg_b[0][0]
        b_y = sorted(p[1] for p in seg_b)

        if a_x[0] <= b_x <= a_x[1] and b_y[0] <= a_y <=b_y[1]:
            return (b_x, a_y)
...
def run():
    ...
    path_strings = [line.strip() for line in INPUT.splitlines()]

    a, b = path_strings

    a_segs = _generate_segments(a)

    b_segs = _generate_segments(b)

    intersections = set()

    for a_seg in a_segs:
        for b_seg in b_segs:
            intersection = _get_intersection(a_seg, b_seg)
            if intersection is not None:
                intersections.add(intersection)

    print intersections

    print min(intersections, key=lambda x: sum(abs(v) for v in x))
